...
function Connection(socket) {
  ...
  this._socket.addListener('data', function(data) {
    ...
    if (!self._jsonStream) {
      ...
      self._jsonStream.on('data', function (obj) {
        //console.log('obj data', obj);
        if (obj.result !== undefined || obj.error !== undefined) {
          self.emit('response', obj);
        } else if (obj.id !== null && obj.id !== undefined) {
...
Connection.prototype.connect = function(port, host, callback) {
  ...
  if (this._jsonStream) {
    this._jsonStream.end();
    this._jsonStream.removeAllListeners();
    delete this._jsonStream;
    //console.log('delete _jsonStream');
  }
};
...
Connection.prototype._handleRequest = function(req) {
  ...
  if (typeof method === 'function') {
    ...
    result(new Error('Method Not Found, method=' + req.method));
  }
}
